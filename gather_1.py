...
# 读取四个CSV文件
df_luo = pd.read_csv('忍者项目记录-每日采集记录（罗圣峰）.csv', encoding='utf-8')
df_zhou = pd.read_csv('忍者项目记录-每日采集记录（周俊健）.csv', encoding='utf-8')
df_he = pd.read_csv('忍者项目记录-每日采集记录（贺子豪）.csv', encoding='utf-8')
df_jiang = pd.read_csv('忍者项目记录-每日采集记录（江伊潭）.csv', encoding='utf-8')
...
# 保存为新的CSV文件
df_merged.to_csv('忍者合并_first.csv', index=False, encoding='utf-8-sig')
